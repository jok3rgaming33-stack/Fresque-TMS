...
export default function CardDetailSheet({
  ...
  return (
    <div className="sheet p-4 md:rounded-sm md:border md:border-[var(--line)] md:p-5">
      <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--gold)]">{card.family}</p>
      <h3 className="mt-1 font-serif text-2xl leading-tight">{card.title}</h3>
      ...
      {card.description && card.description !== card.title ? (
        <p className="mt-3 text-sm leading-relaxed text-[var(--text)]">{card.description}</p>
      ) : null}
      {showZones ? (
